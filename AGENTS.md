...
- `webview/` — React + Vite + Semi Design Webview 源码
  ...
  - `src/shared/` — Webview 侧消息封装、公共样式和内容增强逻辑
...
- Webview 页面必须适配 [VS Code Color Theme](https://code.visualstudio.com/api/references/theme-color)，样式优先使用官方 Theme Color CSS 变量（`var(--vscode-*)`），必要时先在 `webview/src/shared/styles.scss` 定义主题别名或 Semi Design token 映射，再写页面样式
- 帖子页使用 Semi Design 外层组件，V2EX 返回的正文、附言和回复内容继续通过 `dangerouslySetInnerHTML` 渲染
- Webview 通过 `acquireVsCodeApi().postMessage` 与扩展通信，消息类型集中在 `src/shared/webview.ts`
- 扩展侧使用 `src/core/webviewHtml.ts` 读取 Vite 输出 HTML，并将本地 `src` / `href` 转换为 `webview.asWebviewUri(...)`
- 帖子内容增强逻辑在 `webview/src/shared/topicContent.ts`，负责图片预览、隐藏图片占位、站内话题跳转和 imgur 代理
