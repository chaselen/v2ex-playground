...
- `src/features/` — 独立功能模块，如每日签到、图片预览和详情面板导航
...
- `webview/` — React + Vite + Semi Design Webview 源码
  - `main.html` / `topic.html` / `member.html` — Vite 多页面入口
...
- 话题页、用户页和“我的”消息使用 Semi Design 外层组件，V2EX 返回的 HTML 内容通过 `dangerouslySetInnerHTML` 渲染；复用 `webview/src/shared/topicContent.ts` 中的标准化和增强逻辑
- Webview 通过 `WebviewRpcBridge` 封装 `acquireVsCodeApi().postMessage` 与扩展通信，RPC 类型集中在 `src/shared/mainView.ts`、`src/shared/topicView.ts`、`src/shared/memberView.ts` 和 `src/shared/webviewRpc.ts`
- 新增或修改 RPC 命令、事件、请求或响应字段时，先更新 `src/shared/` 中的契约，再同步扩展侧处理器和 Webview 调用方
- 扩展侧使用 `src/core/webviewHtml.ts` 读取 Vite 输出 HTML，并将本地 `src` / `href` 转换为 `webview.asWebviewUri(...)`
- 内容增强逻辑在 `webview/src/shared/topicContent.ts`，负责 HTML 标准化、图片预览、隐藏图片占位、站内话题跳转和用户页内容链接处理
