...
- `webview/` — React + Vite + Semi Design Webview 源码
  - `main.html` / `topic.html` / `member.html` / `balance.html` — Vite 多页面入口
...
- Webview HTML 入口使用 `https://www.v2ex.com/` 作为 `<base>`；Webview 打开外部链接前使用 `resolveWebviewUrl()` 基于 `document.baseURI` 解析为绝对地址，扩展侧统一复用 `src/features/openExternal.ts`
