...
手动和自动签到共用同一个模块级 `dailySignInTask`：
...
`accountKey` 是持久化登录 Cookie 的 SHA-256 指纹，不保存原始 Cookie。读取缓存时必须同时匹配账号指纹和当前 `+08:00` 日期，因此同一天切换账号不会复用另一个账号的签到结果。

旧版本只保存日期字符串。读取新结构时旧数据不会匹配，会通过 V2EX 页面和余额流水重新确认状态。
