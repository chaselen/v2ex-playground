...
- V2EX 每日任务刷新到新一天之前（常见为 UTC 0 点 / 北京时间约 8 点）页面仍可能停留在上一任务日的「已领取」；此时奖励日期可能早于当前 `+08:00` 日期，UI 仍显示已签到，因为当前任务日无法再次领取
- 本地完成缓存只在 `reward.date` 等于当前 `+08:00` 日期时写入，避免在任务尚未刷新时把“上一任务日已领取”误缓存成“今天已完成”，导致任务刷新后自动签到被跳过
...
- 本地缓存：仅在 `success`，或 `repetitive` 且 `rewardDate` 等于当前 `+08:00` 日期时写入

因此每日任务尚未刷新、页面仍显示上一任务日已领取时，Webview 会显示已签到；自动签到不会把该状态缓存为“今天已完成”，任务刷新后仍会继续尝试领取。
...
如果本地缓存已经确认当前账号在当前 `+08:00` 日期完成签到，自动签到会直接返回，跳过对 V2EX 的请求。

若本地尚无今日缓存，自动签到会先调用 `getDailySignInStatus()`：
...
- V2EX 每日任务刷新时间可能与本地 `+08:00` 日历日不一致（常见接近 UTC 0 点 / 北京时间约 8 点）；任务尚未刷新时 UI 按页面已领取展示已签到，本地缓存仍按奖励日期是否属于今天决定是否跳过自动签到
