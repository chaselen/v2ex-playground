...
定期检查不写死某个刷新时间。V2EX 历史规则曾以 UTC 作为每日边界，但页面行为可能变化；按小时检查可以在签到页实际刷新到新一天后自然完成领取。
...
- V2EX 每日任务刷新时间可能与本地 `+08:00` 日历日不一致（历史上接近 UTC 0 点）；任务尚未刷新时 UI 按页面已领取展示已签到，本地缓存仍按奖励日期是否属于今天决定是否跳过自动签到
